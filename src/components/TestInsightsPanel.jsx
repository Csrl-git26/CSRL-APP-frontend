      ...
      {!showStudentCard && (
        <>
          ...
          <div className="grid-2">
                  ...
                  <div key={sub} style={{ background: 'var(--gray-50)', borderRadius: 8, padding: 12 }}>
                    ...
                    <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12, lineHeight: 1.7 }}>
                      {Object.entries((insights.notQualifiedBySubject || {})[sub] || {})
                        ...
                        .sort((a, b) => b[1] - a[1])
                        .map(([code, n]) => (
                          <li key={code} style={{ color: 'var(--red)', fontWeight: 600 }}>
                            {code}: {n}
                          </li>
                        ))}
            ...
            <div className="card">
              <div className="section-title" style={{ fontSize: 14 }}>{insights?.overallTopper?.stream === 'NEET' ? 'Total number of students not qualified for MBBS - centrewise' : 'Not qualified (overall) — count by centre'}</div>
