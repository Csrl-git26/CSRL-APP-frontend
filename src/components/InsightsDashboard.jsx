    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      {showRankingModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.35)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowRankingModal(false)}>
          ...
          <div style={{ background: 'rgba(255, 255, 255, 0.45)', border: '1px solid rgba(255,255,255,0.6)', borderRadius: 24, width: '95vw', maxWidth: 1400, height: '85vh', display: 'flex', gap: 20, boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 0 1px rgba(255,255,255,0.2)', padding: 24, position: 'relative', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }} onClick={e => e.stopPropagation()}>
            
            {/* Close button */}
            <div style={{ position: 'absolute', top: 12, right: 16, cursor: 'pointer', width: 32, height: 32, borderRadius: '50%', background: 'rgba(100,116,139,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }} onClick={() => setShowRankingModal(false)}>
              <span style={{ fontSize: 22, lineHeight: 1, fontWeight: 400, color: '#64748b' }}>\u00d7</span>
            </div>

            {/* Left Card: Centre Rankings */}
            <div style={{ flex: 1, background: '#f8fafc', borderRadius: 16, padding: 24, display: 'flex', flexDirection: 'column', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1)' }}>
              ...
              <div style={{ flex: 1, minHeight: 0, width: '100%' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={[...centreBoard].sort((a,b) => (b.avg||0)-(a.avg||0))} margin={{ top: 30, right: 10, left: 0, bottom: 40 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(0,0,0,0.06)" />
                    <XAxis dataKey="code" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 800, fill: '#1e293b', angle: -90, textAnchor: 'end' }} interval={0} dx={-4} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 800, fill: '#64748b' }} domain={[0, 180]} ticks={[0, 45, 90, 135, 180]} label={{ value: 'Average Score', angle: -90, position: 'insideLeft', style: { fontWeight: 900, fill: '#475569', fontSize: 13 } }} />
                    ...
                    <Bar dataKey="avg" fill="#1e3a8a" radius={[6, 6, 6, 6]} barSize={18}>
                      <LabelList dataKey="avg" content={(props) => {
                        ...
                        return (
                          <g>
                            <text x={x + width / 2} y={y - 12} fill="#1e293b" fontSize={10} fontWeight={900} textAnchor="middle">
                              {Math.round(value)}
                            </text>
                            {isAlert && (
                              <g>
            ...
            <div style={{ flex: 1, background: '#f8fafc', borderRadius: 16, padding: 24, display: 'flex', flexDirection: 'column', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <span style={{ fontSize: 17, fontWeight: 800, color: '#2563eb' }}>Centre Performance Trend</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', padding: '5px 12px', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                   <span style={{ fontWeight: 800, color: '#1e293b', fontSize: 13 }}>{[...centreBoard].sort((a,b) => (b.avg||0)-(a.avg||0))[0]?.code || 'ABB'}</span>
                   <span style={{ fontSize: 10, color: '#94a3b8' }}>\u25bc</span>
                </div>
              </div>
              
              <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                 <div style={{ background: '#3b82f6', color: '#fff', padding: '5px 14px', borderRadius: 20, fontSize: 11, fontWeight: 800, cursor: 'pointer' }}>MARKS</div>
                 {['ACCURACY','ATTEMPTED','CORRECT','RANK'].map(t => (
                   <div key={t} style={{ background: '#fff', color: '#64748b', padding: '5px 14px', borderRadius: 20, fontSize: 11, fontWeight: 700, border: '1px solid #e2e8f0', cursor: 'pointer' }}>{t}</div>
                 ))}
              </div>

              <div style={{ display: 'flex', gap: 16, marginBottom: 16, background: '#fff', padding: '8px 16px', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                 {[{s:'Physics',c:'#3b82f6'},{s:'Chemistry',c:'#f97316'},{s:'Math',c:'#10b981'},{s:'Total',c:'#8b5cf6'}].map(({s,c}) => (
                   <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                     <div style={{ width: 14, height: 14, borderRadius: 3, background: c, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                       <span style={{ color: '#fff', fontSize: 9, fontWeight: 900 }}>\u2713</span>
                     </div>
                     <span style={{ fontSize: 13, fontWeight: 700, color: '#475569' }}>{s}</span>
                   </div>
                 ))}
              </div>
              ...
              <div style={{ flex: 1, minHeight: 0, width: '100%' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={(() => {
                    const topCode = [...centreBoard].sort((a,b) => (b.avg||0)-(a.avg||0))[0]?.code;
                    const testNames = tests.map(t => t.testName || t.name).filter(Boolean);
                    return testNames.map(tName => {
                      const row = { name: tName };
                      const cData = centreBoard.find(c => c.code === topCode);
                      row.total = Math.round(cData?.avg || 0);
                      row.physics = Math.round((cData?.avg || 0) * 0.35);
                      row.chemistry = Math.round((cData?.avg || 0) * 0.30);
                      row.math = Math.round((cData?.avg || 0) * 0.35);
                      return row;
                    }).slice(-7);
                    ...
                    <Line type="monotone" dataKey="chemistry" stroke="#f97316" strokeWidth={2} dot={{ r: 4, fill: '#fff', stroke: '#f97316', strokeWidth: 2 }} />
                    <Line type="monotone" dataKey="math" stroke="#10b981" strokeWidth={2} dot={{ r: 4, fill: '#fff', stroke: '#10b981', strokeWidth: 2 }} />
                    <Line type="monotone" dataKey="physics" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4, fill: '#fff', stroke: '#3b82f6', strokeWidth: 2 }} />
                    <Line type="monotone" dataKey="total" stroke="#8b5cf6" strokeWidth={2.5} dot={{ r: 5, fill: '#fff', stroke: '#8b5cf6', strokeWidth: 2 }} />
                    <Legend verticalAlign="bottom" height={36} wrapperStyle={{ fontSize: 12, fontWeight: 700, paddingTop: 10 }} iconType="circle" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
